...
<jsp:include page="/header.jsp" />
			...
			<div class="main-content">
				<c:set var="currentlist" value="${booklist}" scope="session" />
				<p>
					Sort by price: 
					<c:choose>
						<c:when test="${param.direction == 'desc' or empty param.direction}">
							<a href="<c:url value="/book/showsorted?direction=asc" />">from lowest to highest</a>
						</c:when>
						<c:otherwise>
							<a href="<c:url value="/book/showsorted?direction=desc" />">from highest to lowest</a>
						</c:otherwise>
					</c:choose>
				</p>
				<table>
